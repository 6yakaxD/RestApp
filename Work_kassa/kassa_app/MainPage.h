...
public:
    ...
    // [ VARS FOR CHANGINGS ]
    bool    Is_SettingsWidget_Open      = false;  // открыт ли виджет настроек
    bool    Is_SeatPlaceWidget_Open     = false;  // открыт ли виджет выбора места
    bool    Is_DishDescription_Open     = false;  // открыт ли виджет описания блюда
    bool    Is_NewGuestWidget_Open      = false;  // открыт ли виджет добавления клиента 
    bool    Is_GuestRegisteredForOrder  = false;  // создан ли клиент для заказа
    bool    Is_PaymentWidget_Open       = false;  // открыт ли виджет оплаты
    bool    Is_OrderPaidOkay            = false;  // оплачен ли заказ
    int     Dish_type                   = 0;      // тип блюда {5}
    int     Dish_id                     = 0;      // id блюда  {}
    int     Place_id                    = 0;      // id места
        ...
        // [ Left Menu ]
        {
            ...
            // [ Dish Description ]
            {
                ...
                connect(m_DishDescription_ShowToCustomerButton, &QToolButton::clicked, [=]() {
                    //  вывести на монитор картинку товара
            ...
            // [ MAIN Type Dishes]
            {
                ...
                // пропорциональное расстояние между строками/столбцами
                ...
                // [ Type 1 Dishes ]
                {
                    ...
                    connect(m_dishType1_1, &QToolButton::clicked, [=]() {
                        Dish_id = 1;                            // тип блюда - 1
                        m_DishDescription_Widget->show();       // открыть описание блюда
                        set_DishOptions(Dish_id);               // присвоить название и описание блюда 
                        close_for_dishDescription(Dish_type);   // закрыть все что было на фоне 
                        Is_DishDescription_Open = true;         // открыто описание
                    ...
                    connect(m_dishType1_BACK_Button, &QToolButton::clicked, [=]() {
                        close_for_dishDescription(Dish_type);   // закрыть все что было на фоне
                        m_left_MAINBackWidget->show();          // показать главные типы меню 
                        Dish_type = 0;                          // тип блюда = 0
                        });
                }
    ...
    void    addRow(int id) 
    {
        ...
        item->setFlags(item->flags() & ~Qt::ItemIsEditable); // Запрет изменения данных
        item->setFlags(item->flags() & ~Qt::ItemIsSelectable); // запрещает выделять
    ...
    // +-------------------------[ Left Menu ]-----------------------------+
    QWidget*        m_left_MAINBackWidget;      // главный задний виджет всех типов блюд
    QGridLayout*    m_left_MAINGridLayout;      // его layout
    QToolButton*    m_dishType1;                // тип блюда 1
    QToolButton*    m_dishType2;                // тип блюда 2
    QToolButton*    m_dishType3;                // тип блюда 3
    QToolButton*    m_dishType4;                // тип блюда 4
    QToolButton*    m_dishType5;                // тип блюда 5

    QToolButton*    m_left_DeleteGuest_Button;  // кнопка удаления клиента

    // 1 Type
    QWidget*        m_left_1_TypeBackWidget;    // виджет 1 типа блюда
    QGridLayout*    m_left_1_TypeGridLayout;    // его layout
    QToolButton*    m_dishType1_1;              // тип блюда 1 блюдо 1
    QToolButton*    m_dishType1_2;              // тип блюда 1 блюдо 2
    QToolButton*    m_dishType1_3;              // тип блюда 1 блюдо 3
    QToolButton*    m_dishType1_BACK_Button;    // тип блюда 1 блюдо 4

    // 2 Type
    QWidget*        m_left_2_TypeBackWidget;    // виджет 2 типа блюда
    QGridLayout*    m_left_2_TypeGridLayout;    // его layout
    QToolButton*    m_dishType2_1;              // тип блюда 2 блюдо 1
    QToolButton*    m_dishType2_2;              // тип блюда 2 блюдо 2
    QToolButton*    m_dishType2_3;              // тип блюда 2 блюдо 3
    QToolButton*    m_dishType2_BACK_Button;    // тип блюда 2 блюдо 4

    // 3 Type
    QWidget*        m_left_3_TypeBackWidget;    // виджет 3 типа блюда
    QGridLayout*    m_left_3_TypeGridLayout;    // его layout
    QToolButton*    m_dishType3_1;              // тип блюда 3 блюдо 1
    QToolButton*    m_dishType3_2;              // тип блюда 3 блюдо 2
    QToolButton*    m_dishType3_3;              // тип блюда 3 блюдо 3
    QToolButton*    m_dishType3_BACK_Button;    // тип блюда 3 блюдо 4

    // 4 Type
    QWidget*        m_left_4_TypeBackWidget;    // виджет 4 типа блюда
    QGridLayout*    m_left_4_TypeGridLayout;    // его layout
    QToolButton*    m_dishType4_1;              // тип блюда 4 блюдо 1
    QToolButton*    m_dishType4_2;              // тип блюда 4 блюдо 2
    QToolButton*    m_dishType4_3;              // тип блюда 4 блюдо 3
    QToolButton*    m_dishType4_BACK_Button;    // тип блюда 4 блюдо 4
    
    // 5 Type
    QWidget*        m_left_5_TypeBackWidget;    // виджет 5 типа блюда
    QGridLayout*    m_left_5_TypeGridLayout;    // его layout
    QToolButton*    m_dishType5_1;              // тип блюда 5 блюдо 1
    QToolButton*    m_dishType5_2;              // тип блюда 5 блюдо 2
    QToolButton*    m_dishType5_3;              // тип блюда 5 блюдо 3
    QToolButton*    m_dishType5_BACK_Button;    // тип блюда 5 блюдо 4
